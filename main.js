...
gltfLoader.load(
  ...
  function (room) {
    ...
    video.src = 'textures/monitor-video.mp4';
    video.muted = true;
    ...
    room.scene.children.forEach((child) => {
      ...
      if (child.name === 'CPU') {
        ...
        child.children[0].material.color.set(0xfffff);
...
function loadIntroText() {
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const titleGeo = new TextGeometry('SUSHIGUY', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Web Designer / Developer / Tech Enthusiast',
      {
        font: font,
        size: 0.018,
        height: 0,
